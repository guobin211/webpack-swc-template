 ...
 * @param path
 */
export function getFlattenedTreePathsAndParent(tree: TreeLikeData[], parents: ParentsInfo = [], path: PathInfo = []): FlattenedTreeNode {
...
function getCheckState(node: TreeNode) {
    if (node.children && node.children.length > 0) {
        ...
        return {
            ...
            halfChecked: checkedNum > 0 && checkedNum < node.children.length || halfChecked > 0,
        };
    }
    return node.state;
}
